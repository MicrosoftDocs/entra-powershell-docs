...
description: "Learn how to review, assign, and remove Microsoft Entra user licenses using Microsoft Entra PowerShell. Optimize license costs and audit assignments across your tenant."
...
#customer intent: As an admin, I want to manage the assignment of Entra licenses to users using Microsoft Entra PowerShell so that I can optimize license costs and ensure appropriate access across my tenant.
---

# Manage user licenses using Microsoft Entra PowerShell

This article shows you how to use Microsoft Entra PowerShell to manage user licenses in your organization. You learn how to review and audit license assignments, assign licenses to individual and multiple users, remove licenses, and manage inactive user licenses.
...
Before assigning licenses, confirm which licenses are already assigned to the user.
...
To assign a license to a user, use the `Set-EntraUserLicense` cmdlet, which adds or removes licenses from a user's assignment.
...
To assign more than one license to a user, create an array of `AssignedLicense` objects and add them to the `AssignedLicenses` object. This script assigns the `AAD_PREMIUM_P2` and `EMS` license plans to a user.
...
This script assigns multiple licenses to multiple users simultaneously. You can define the list of users directly in the script or import them from a CSV file.
...
Auditing licenses for inactive users helps reduce costs and improve security. To list inactive users with active licenses, run this command:
