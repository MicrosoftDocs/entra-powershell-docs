...
1. Microsoft Entra PowerShell is installed. Follow the [Install the Microsoft Entra PowerShell][install] guide to install the module.
1. Microsoft Entra PowerShell using a **Privileged Role Administrator** in the tenant and the appropriate permissions. For this tutorial, the `Application.Read.All` and `AppRoleAssignment.ReadWrite.All` delegated permissions are required. To set the permissions in Microsoft Entra PowerShell, run:
...
New-EntraServicePrincipalAppRoleAssignment -ServicePrincipalId '2cab1707-656d-40cc-8522-3178a184e03d' -BodyParameter $params | 
...
```powershell
Remove-EntraServicePrincipalAppRoleAssignedTo -ServicePrincipalId '22c1770d-30df-49e7-a763-f39d2ef9b369' -AppRoleAssignmentId 'DXfBIt8w50mnY_OdLvmzaUbMIDgaM6pCpU8rpQHnPf0'
```
::: zone-end
